...
# Sort items by value
sorted_items = sorted(colours.items(), key=lambda x: x[1])
...
if n % 2 == 1:
    ...
    median_colour = sorted_items[n // 2][0]
else:
    # Even number of values, take the middle two, and select the first one (you can choose which strategy to use here)
    median_colour = sorted_items[n // 2 - 1][0]
